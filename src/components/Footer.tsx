...
const Footer: React.FC = () => {
  ...
  const monogram = `${WEDDING_DATA.couple.groom.firstName.charAt(0)} & ${WEDDING_DATA.couple.bride.firstName.charAt(0)}`;
      ...
      <div className="max-w-7xl mx-auto relative z-10 h-full flex flex-col justify-center px-6">
        ...
        <div className="flex-grow flex items-center">
            <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 border-b border-white/20 pb-12">
              ...
              <div className="footer-item flex flex-col items-center md:items-start text-center md:text-left space-y-4">
                <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-stone-300 mb-2">Explore</h3>
                <a href="#couple" className="font-serif text-xl hover:text-stone-300 hover:italic transition-all duration-300">The Couple</a>
                <a href="#roundown-acara" className="font-serif text-xl hover:text-stone-300 hover:italic transition-all duration-300">The Schedule</a>
                <a href="#gallery" className="font-serif text-xl hover:text-stone-300 hover:italic transition-all duration-300">Moments</a>
                <a href="#ucapan-rsvp" className="font-serif text-xl hover:text-stone-300 hover:italic transition-all duration-300">RSVP</a>
              </div>

              {/* Column 2: Main Message */}
              <div className="footer-item flex flex-col items-center text-center space-y-6">
                <h2 className="font-serif text-4xl md:text-5xl text-stone-50 italic font-medium">Thank You</h2>
                <p className="font-sans text-sm md:text-base leading-relaxed text-stone-200 max-w-sm">
                    It is an honor and happiness for us if you are willing to attend and give your blessings to us. 
                    We look forward to celebrating with you in {WEDDING_DATA.hero.location.split(',')[0]}.
                </p>
              ...
              <div className="footer-item flex flex-col items-center md:items-end text-center md:text-right space-y-6">
                <div>
                    <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-stone-300 mb-3">Wedding Hashtag</h3>
                    {WEDDING_DATA.footer.hashtags.map((tag, idx) => (
                        <p key={idx} className="font-serif text-2xl text-stone-100">{tag}</p>
                    ))}
                </div>
                
                <div className="pt-4">
                    <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-stone-300 mb-3">Follow Us</h3>
                    <div className="flex gap-4 justify-center md:justify-end">
                        {WEDDING_DATA.footer.socialLinks.map((link, idx) => (
                            <a key={idx} href={link.url} target="_blank" rel="noreferrer" className="p-2 border border-white/20 rounded-full hover:bg-white hover:text-stone-900 transition-colors">
        ...
        <div className="py-8 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          >
             ...
             <span className="text-[9px] uppercase tracking-widest text-stone-400 group-hover:text-stone-200 transition-colors">Back to Top</span>
          </button>
